...
The superheterodyne receiver is often found in the literature with short forms like \textit{superhet}, or simply \textit{super}.
...
The IF is usually set to a fixed frequency.
This results in the advantage, that the subsequent IF filter can be optimized, since it does not have to be flexible in frequency.

Fig.\ref{fig:bd_superhet} shows a block diagram of a superhet receiver.
The antenna signal first runs through an RF amplifier and filter.
It selects the desired frequency band and filters out any unwanted signals outside of this band.
The next step is the mixer, which is fed by a local oscillator.
This shifts, or down-converts, the antenna signal down to the IF.
Afterwards, the IF signal is amplified and filtered again.
This is to remove the side-products of the mixer and may also be called as image rejection filter.
The demodulator stage can now perform any further signal processing on the signal, until the required signal is retrieved.
In the shown case, which may be an FM broadcast receiver, an audio output is generated.

\includepicture [1.0] [0] {Block design of a superhetorodyne receiver.} {bd_superhet} {img/superheterodyne-receiver-block-diagram.png}
...
The process of down-conversion is used to convert an RF signal to a signal of lower frequency, while preserving its information content.
...
math. formulas\\
digital: meet nyquist criteria, just like any other DSP (show in spectrum)\\
