...
In the development of a software project, the metric for the lines of code is sometimes used to give an idea of the scale, or volume of the project.
The time it takes to implement a certain set of features can also give such an insight and a link to the project's complexity.
It is a subjective metric though, since it is influenced by the respective developer's experience in the specific application.
However, both metrics are applied to the FM Radio Receiver project.\\

At this point, it is important to note that the results that presented in this section are very specific to the chosen architecture, testbench framework, and the implementation style generally.
...
Interesting details can be found by analyzing the charts.
The upper chart reveals that the VHDL implementation with its accompanying testbench requires more than twice the amount of code, compared to the HLS version.
The lower charts also highlight several differences between HLS and VHDL.
The code proportions between IP design and testbench show, that HLS requires about the same amount for these parts, while VHDL requires the largest part of code in the IP design.


\begin{figure}%
  \centering
  \subfloat[\centering Overview of the proportions between the Matlab model, the HLS and VHDL IPs.]{{\includegraphics[trim=10 10 10 10,clip, width=0.5\textwidth]{img/matlab/lines_of_code_pie_chart_py_all} }}%
  \\
  %\hspace{\fill}
  ...
  \caption{Lines of code of the respective system design parts, presented in multiple pie charts. The percentages of code in IP design and testbench for the HLS and VHDL implementations are shown in detail.}%
...
Also note, that the common Python testbench consists of XXX lines of Python code.
Therefor, the testbench code of the respective IPs is relatively small, because the entire analysis part is shared.
