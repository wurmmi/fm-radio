...
This chapter presents the results that are achieved in the development of the FM radio receiver project, with the chosen implementation methods.
The main focus is to compare the results that are achieved specifically with HLS and VHDL, since these are the major implementation variants of the project.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{General}

The main objective of this thesis is to implement an FM radio receiver in multiple different methods, within a common system design.
...
This is mainly due to the different levels of abstraction, so that the low-level algorithms do not always need to be known into detail.\\
...
In other words, anything that is implemented in HLS, can also be implemented in VHDL directly.
...
The implementation of the FM receiver is split into two main parts, the communication interfaces and the DSP chain.
...
There are read-only and read-writeable registers, which are all mapped into a common register-map, that starts from a single base address, as they are accessible through this bus interface.\\
...
This includes the comparison with the testbench simulation, as well as the results from the execution on actual hardware.
...
%%%%%
\subsubsection{Comparison against the Matlab reference}
...
This is visible in the analysis of the audio signal, as shown in Figure~\ref{fig:audio_output_compare_tb_vs_hw}.
...
This includes the carrier recovery, i.e. the 38~kHz subcarrier, as well as a sample time-shift in the final summation and substraction to recover the left and right channels.
...
However, from the system-level point of view of this thesis, the IP works and can be integrated into the final system successfully.
...
%%%%%
\subsubsection{Comparison of simulation- against hardware results}

The results of the simulation testbenches are compared against the values that are read from the FPGA directly.
Ideally, the values should be exactly the same.
However, this is not always the case, as Figure~\ref{fig:audio_output_compare_tb_vs_hw} demonstrates.
The HLS implementation does have a matching result between simulation and hardware, whereas the VHDL variant has deviating values.
The explanation here may be linked with the previously mentioned suspicions, but may also be linked to the initial reset values that exist in the hardware.
...
Any adaptions in the VHDL design can be simulated in the comprehensive testbench, and subsequently be integrated into the FPGA.
All these processes are automated with scripts, so that, besides the actual work of interest in the HDL design, no manual actions are necessary.
The new results can then be analyzed and compared again, which may lead to further adaptions and eventually resolve any existing issue.
...
Thus, a generalization in a manner of \textit{HLS always requires less code than VHDL} can not be stated.
However, the results in this section can give an idea and overview of this topic and provide a detailed and valid comparison for this specific project.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Lines of Code}
\label{sec:results:lines-of-code}

Several parts of the system design are taken into account for the analysis of the lines of code, to give an impression over the largest code parts in the project.
...
Table~\ref{tab:lines_of_code} displays the actual numbers of code lines, to quantify the size of these respective parts.\\
...
VHDL on the other hand, requires the largest part of code in the IP design, while the testbench code amount is relatively small.
...
\begin{figure}%
  \centering
  \subfloat[\centering Overview of the proportions between all the implemented code parts of the entire project.]{{\includegraphics[trim=10 40 10 10,clip, width=0.68\textwidth]{img/matlab/lines_of_code_pie_chart_py_all} }}%
  \\
  %\hspace{\fill}
  \subfloat[\centering HLS in detail.]{{\includegraphics[trim=50 10 15 10,clip, width=0.49\textwidth]{img/matlab/lines_of_code_pie_chart_py_hls} }}%
  %\hspace{\fill}
  %\vspace{3cm}
  \subfloat[\centering VHDL in detail.]{{\includegraphics[trim=50 10 15 10,clip, width=0.49\textwidth]{img/matlab/lines_of_code_pie_chart_py_vhdl} }}%

  \caption{Lines of code of the respective parts in the entire project, presented in multiple pie charts. The percentages of code in IP design and testbench for the HLS and VHDL implementations are presented in detail in the lower diagrams, where the legends show the respective implementation languages of the sub-parts in the inner circles.}%
...
In the present case, there is a strong background of knowledge and practical experience in VHDL design, while there is only very little thereof in HLS design.
Thus, the logged HLS times also include the time that is spent in research, which is required to implement a specific feature.
However, this metric is used in this section to specifically compare HLS with VHDL for this project.
Certain features are chosen for this comparison and their implementation times are logged during the actual implementation process.
...
The following paragraphs present an insight into how these implementation times of the listed code parts arise.
...
In HLS, it can be automatically inferred by adding pragma directives to the respective function parameter variables, and the testbench code is implemented like a regular function call.
...
It is to note, that this comparison does not compare functionally equivalent implementations though, because the actually implemented logic in VHDL does not use the ready-flag correctly, as explained in the previous chapters.
According to this fact, an even further increase in implementation hours should actually be considered for VHDL.
However, also the HLS implementation has its particular characteristics that need to be understood for a fully functional implementation, which requires time in research.
...
To create this process more efficiently and extendable, a Python script -- the so-called Register Engine -- is used, which can also automatically generate VHDL code for the registers' memory map.
This turns out to be a multiplicator in productivity for the implementation of registers.
The register generator script was already used in previous projects and is therefore well-known and tested, before it is used in this project.
...
Even though the register generator script already generates a \cplusplus\ header file, which includes several address offset definitions of the registers, the actual driver functions need to be implemented manually.
Furthermore, several files need to be integrated into a pre-defined folder structure that Vivado understands, in order to enable a correct integration of the IP in a Vivado block diagram.\\
...
The DSP mode register can select between the regular FM radio mode and a pass-through mode, which simply forwards the streaming data without any processing.\\
...
For the HLS version, a \cplusplus\ code example of an FIR filter, which is provided by Xilinx HLS, is taken and modified for the present purpose.
...
The VHDL implementation is straightforward, in comparison.
...
The initial version was implemented like the VHDL version -- a counter that only triggers the subsequent DSP blocks upon a certain number of samples is received.
...
By a thorough investigation in several user guides, in combination with step-by-step debugging, the issue was found and led to the current, working implementation, which processes a number of samples, before it forwards one sample.
...
The implementation of the receiver structure requires the combination of several previously implemented blocks, such as the FIR filter instances and any interfaces.
...
For example, if two parallel FIR filters, which have a different filter order and thus, a different group delay, produce output signals that are to be added, they need to be aligned, so that they are valid at the time of the summation.
...
Another factor that adds up to the implementation time, is the number of lines of code, as presented in Section~\ref{sec:results:lines-of-code}.
...
In the final summation over several implementation times of all the listed features, the two variants HLS and VHDL only differ by 10 hours, which is about 15\%.
...
Nevertheless, even considering this fact, the HLS implementation is faster than the VHDL equivalent.
...
It is important to note, that the y-axis has a logarithmic scale, as a method to enable the presentation of the values in multiple orders of magnitude in a single diagram.
...
In the current case, there is also an additional interface between the VHDL simulator and the Python testbench framework, which likely slows down the simulation even further.
The HLS testbench in comparison achieves around 50.000 samples per second, which is a speed-up factor of 2.500.
This is specifically talking about the HLS C/\cplusplus\ simulation.
The main reason for the speed-up is, that this type of simulation does not simulate any timing behaviour -- any processes are treated as C/\cplusplus\ functions, which run like a regular executable on the host CPU.
Also, the HLS compiler is, where possible, using native \cplusplus\ data types to represent the signals, which results in the faster execution of the code.
...
Consequently, Matlab runs very efficiently and achieves the highest simulation speed of around 3~million samples per second.
...
Because of this modular and structured approach, given by this system design, it can be applied to almost any DSP project.
...
These IP blocks should be able to be connected with their AXI interfaces, in order to successfully communicate and interact with the embedded hard-processor, the ARM CPU, as well as other IPs that exist in the design.
...
This is a non-documented requirement, and furthermore, for issues like that, the Eclipse-based SDK does not give any error output or any kind of meaningful information.
...
However, both IPs could successfully be integrated into the Vivado block design in the end, and the tests on the hardware show that they function correctly within the system.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Hardware Utilization}

The hardware utilization of the HLS and VHDL implementation is compared in detail in this section.
The numbers are taken from the final utilization report, that is created by Vivado at the end of the bitstream creation, i.e. post-synthesis, since this is assumed to be the most accurate report, which shows the actual utilization, as it is being deployed to hardware.
...
The study of the detailed utilization report, however, does not enable a deeper direct comparison of the respective single blocks, such as VHDL FIR filter versus HLS FIR filter.
This is because the HLS compiler performs some kind of name mangling, which re-names functions, and automatically applies optimizations, so that the direct connection between code and resource report is partially lost.
...
Generally in hardware design, there is always a trade-off between resource utilization and throughput, which ultimately leads to time-sharing.
...
In HLS, this trade-off can be influenced and determined by the developer, by the usage of optimization directives.
Depending on the requirements of the design, the optimization may either be set towards a low resource utilization, or towards a high throughput.
...
Nevertheless, the resource report allows the direct comparison of one specific unit, the configuration interface, which is the AXI4-Lite interface.
In VHDL, it requires 44~LUTs and 54~FFs, while HLS only consumes 36~LUTs and 37~FFs.
It seems, that the manual VHDL implementation requires more resources in all the comparable parts.
...
%This section is specifically (TODO)
% - show that the transmitter works (show photo of radio display in my car)
% - show that the receiver works (screenshot of decoded RDS data, etc.)
