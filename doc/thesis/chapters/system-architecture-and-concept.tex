...
\includepicture [1.0] [0] [H] {High-level overview of the system, representing the signal flow from the antenna, via the front end to the FM receiver, which is implemented in multiple variants, to the speakers for audio output.} {bd_system_overview_highlevel} {img/draw.io/bd_system_overview_highlevel}
...
\includepicture [0.95] [0, trim=14 0 0 4,clip] {Block diagram of the overall system concept, with common input source, multiple receiver implementations, and a common verification entity.} {bd_system_overview_detailed} {img/draw.io/bd_system_overview_detailed}
%Block diagram of the overall system concept. A common input source is used for the multiple implementations. The generated output files, including the used parameters, are verified in a common entity.
