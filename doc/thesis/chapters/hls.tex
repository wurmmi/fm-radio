...
\includepicture [1.0] [0] {Design Time vs. Application Performance, if the same functionality is implemented in different methods for various targets \cite[Figure 1-1]{VivadoUgHLSIntro}.} {timeline_performance_first_prototype} {img/draw.io/timeline_performance_first_prototype}
...
Xilinx is the largest manufacturer of FPGAs globally and therefore its tool called \textit{Vivado HLS} is widely used.
...
\begin{itemize}
  ...
  \item \textbf{Co-Simulation}\\
      ...
      However, the issue is that the C simulation is executed like a regular program, which means it is executed sequentially, instruction by instruction and thus parallelism can not be represented.\\
...
More detailed information can be found in the respective Xilinx user guide \cite{VivadoUgHLS}.

%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Data Types}

HLS supports special data types, which are not available in standard \cplusplus.
The focus of these data types is mainly to support an optimized translation into RTL, to achieve the least amount of logic cells in the FPGA.\\
...
One example therefor is the DSP48 macro cell that Xilinx FPGAs have built-in.
They support the direct multiplication of two vectors, with a bitwidth of up to 18~bit each.
In an example application, the achievable maximum value of a variable might be limited, and thus, able to fit into a range of 17~bit for the multiplication.
...
In that way, the above example can be optimized to use a data type that actually has a bitwidth of 17~bit, which is exactly the bitwidth that is required for the application.
This results in a lower logic utilization, which in turn results in a higher maximum clock frequency that can be used.
Consequently, more functionality can be implemented and packed into the FPGA device.
...
This is used to provide additional information to the HLS compiler, to assist and instruct it on how to implement certain parts of the code in RTL.
...
Since the topic of optimization is very comprehensive, it cannot be covered into more detail here.
...
This may be another IP that processes data, but may also be a CPU that reads status information or configures the IP's settings.\\
...
The port-level interfaces on the other hand are specific to input and output (IO) ports.
...
This is an especially useful function, since the implementation of the actual interface logic is abstracted from the developer.
...
The code example in Program~\ref{prog:axi4_lite} shows the specification of an AXI4-Lite memory-mapped slave interface, by using pre-processor pragma definitions.
...
Within the code, the interface is used like regular \cplusplus\ variables.\\

The second code example, Program~\ref{prog:axi4_stream}, demonstrates the equally simple definition of an AXI4-Stream interface.
...
Thus, values can only be written into it when there is an empty spot, and can only be read if there is at least one value available.
...
\begin{program}
  \caption{Implementation of an AXI4-Stream interface. The stream is accessed with read- and write-functions.}
...
The general significance of the simulation testbench for an HDL design is explained here, as well as the different levels of abstraction in which an HLS testbench can be executed.
...
Section~\ref{sec:hls:workflow} already mentioned the topics of testbenches and simulation types briefly, in the context of workflow.
The following descriptions are a comprehensive extension around these topics.
...
Thus, in order to allow debugging and verification of HDL design code, the technology of simulation was invented.
Here, the time-consuming step of synthesis is not taken.
Instead, the HDL simulator software is able to simulate the code in a way, that can represent parallel processes, just like they exist in hardware.
...
Of course the HDL language provides support with different data types, that abstract the binary number representation and make the numbers human-readable.
However, it remains an enormous effort to manually write testing code to verify bus interfaces, for example, because they consist of a large number of signals.
In order to simplify such complex verification operations, various testbench frameworks and libraries are available to developers.
...
This means, that the HLS IP core's top-level function is used as a regular function call in the testbench.\\
...
\noindent
This type of simulation takes a significantly longer time to execute, because timing information now actually needs to be considered for the RTL code, which is done by using an HDL simulator.
